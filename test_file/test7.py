...
def detect_all_landmarks(image, mp_pose):
    ...
    if results.pose_landmarks:
        for idx, landmark in enumerate(results.pose_landmarks.landmark):
            ...
            all_landmarks.append((mp.solutions.pose.PoseLandmark(idx).name, (x, y)))

            # 원 그리기
            cv2.circle(image, (x, y), 5, (0, 255, 0), -1)

            # 좌표와 랜드마크 이름 출력
            cv2.putText(image, f'{mp.solutions.pose.PoseLandmark(idx).name} : {x}, {y}', (x, y - 10),
...
if __name__ == "__main__":
    ...
    output_folder = "output_results-8"
    create_folder(output_folder)
